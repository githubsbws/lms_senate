...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        <h1>ผลลัพธ์การสแกนข้อความ</h1>
        <ul class="grid w-full gap-6 md:grid-cols-2">
            <li class="flex flex-col">
                <input type="radio" id="hosting-small" name="hosting" value="hosting-small" class="hidden peer" required />
                <label for="hosting-small" class="flex-grow inline-flex items-center justify-between w-full p-5 text-gray-500 bg-white border border-gray-200 rounded-lg cursor-pointer dark:hover:text-gray-300 dark:border-gray-700 dark:peer-checked:text-blue-500 peer-checked:border-blue-600 peer-checked:text-blue-600 hover:text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700">
            ...
            <li class="flex flex-col">
                ...
                <div class="editor" contenteditable="true" id="editor">
                    @foreach($ocrResults as $result)
                        @php
                            $responseBody = json_decode($result->getBody(), true);  // แปลง response body เป็น array
                            $wordsBlockList = $responseBody['result']['words_block_list'] ?? [];
                        @endphp

                        @foreach($wordsBlockList as $block)
                            <div>
                                <p>{{ $block['words'] }}</p>
                            </div>
                        @endforeach
                ...
                <button class="button" id="saveButton">บันทึกข้อความ</button>
            </li>
        </ul>
        <div class="fixed bottom-0 left-0 z-50 w-full h-16 bg-white border-t border-gray-200 dark:bg-gray-700 dark:border-gray-600">
            <div class="grid h-full max-w-lg grid-cols-4 mx-auto font-medium">
                <a class="inline-flex flex-col items-center justify-center px-5 hover:bg-gray-50 dark:hover:bg-gray-800 group" href="{{url('/upload')}}">
                    ...
                    <span class="text-sm text-gray-500 dark:text-gray-400 group-hover:text-blue-600 dark:group-hover:text-blue-500">Google</span>
                </a>
                <a type="button" class="inline-flex flex-col items-center justify-center px-5 hover:bg-gray-50 dark:hover:bg-gray-800 group" href="{{route('aws.ocr.form')}}">
                    ...
                    <span class="text-sm text-gray-500 dark:text-gray-400 group-hover:text-blue-600 dark:group-hover:text-blue-500">AWS</span>
                </a>
                <a class="inline-flex flex-col items-center justify-center px-5 hover:bg-gray-50 dark:hover:bg-gray-800 group" href="{{route('huawei.ocr')}}">
                    ...
                    <span class="text-sm text-gray-500 dark:text-gray-400 group-hover:text-blue-600 dark:group-hover:text-blue-500">Huawei</span>
                </a>
                <a type="button" class="inline-flex flex-col items-center justify-center px-5 hover:bg-gray-50 dark:hover:bg-gray-800 group" href="{{route('inet.ocr')}}">
                    <svg class="w-5 h-5 mb-2 text-gray-500 dark:text-gray-400 group-hover:text-blue-600 dark:group-hover:text-blue-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M10 0a10 10 0 1 0 10 10A10.011 10.011 0 0 0 10 0Zm0 5a3 3 0 1 1 0 6 3 3 0 0 1 0-6Zm0 13a8.949 8.949 0 0 1-4.951-1.488A3.987 3.987 0 0 1 9 13h2a3.987 3.987 0 0 1 3.951 3.512A8.949 8.949 0 0 1 10 18Z"/>
                    </svg>
                    <span class="text-sm text-gray-500 dark:text-gray-400 group-hover:text-blue-600 dark:group-hover:text-blue-500">INET</span>
                </a>
            </div>
        </div>
        <script>
            document.getElementById('saveButton').addEventListener('click', function() {
                ...
                // ส่งข้อมูลไปยังเซิร์ฟเวอร์หรือทำสิ่งที่ต้องการ
                console.log(content);

            ...
            <main>
                ...
                <div class="section">
                    <div class="section-header mb-3">
                        <h5 class="mb-2"></h5>
                        <form  action="{{ route('admin.document_cate',['id' => $id]) }}" method="POST" enctype="multipart/form-data" class="max-w-sm mx-auto">
                            @csrf
                        <div class="row g-3 mb-2">
                            <div class="col">
                                <label for="name" class="form-label">ประเภทการประชุม</label>
                                <input type="text" name="cate" class="form-control" id="name">
                            </div>
                            <div class="col">
                            ...
                            <div class="card-body">
                                <table id="searchTable" class="table table-striped table-bordered" style="width:100%">
                                    ...
                                    <tbody>
                                        @foreach($cate as $ca)
                                        <tr>
                                            <td class="text-center">{{ $ca->period->name_type_period ?? '-' }}</td>
                                            <td class="text-center">{{ $ca->years->name_type_years ?? '-' }}</td>
                                            <td class="text-center">{{ $ca->name_type_cate ?? '-' }}</td>
                                            <td class="text-center"><a class="fa-solid fa-square-plus me-1" title="เพิ่มครั้งที่ประชุม" href="{{route('admin.document_meet',['id'=>$ca->id])}}"><i></i>เพิ่มครั้งที่ประชุม</a></td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                </table>
